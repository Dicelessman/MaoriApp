...
export const UI = {
    ...
    async init() {
        try {
            ...
            try {
                const links = ['presenze.html', 'dashboard.html', 'calendario.html', 'esploratori.html', 'staff.html', 'audit-logs.html', 'costotrasporti.html', 'costostruttura.html', 'costocambusa.html', 'preventivo.html', 'scadenze.html'];
                links.forEach(href => {
    ...
    async loadSharedComponents() {
        try {
            ...
            if (sharedHeader) sharedHeader.innerHTML = headerHtml;
        ...
        if (loginForm && !loginForm._bound) {
            ...
            loginForm.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    else if (error.code === 'auth/user-disabled') msg = 'Utente disabilitato.';
                    else if (error.code === 'auth/user-not-found') msg = 'Utente non trovato.';
                    else if (error.code === 'auth/wrong-password') msg = 'Password errata.';
    ...
    showConfirmModal({ title, message, confirmText = 'Conferma', cancelText = 'Annulla', onConfirm, onCancel }) {
        ...
        if (finalCancelBtn) finalCancelBtn.addEventListener('click', () => {
            this.closeModal('confirmModal');
    ...
    setupModalEventListeners() {
        ...
        if (confirmDeleteStaffButton && !confirmDeleteStaffButton._bound) {
            ...
            confirmDeleteStaffButton.addEventListener('click', async () => {
                if (!this.currentUser) { this.showToast('Devi essere loggato per eliminare staff.', { type: 'error' }); return; }
        ...
        if (editStaffForm && !editStaffForm._bound) {
            ...
            editStaffForm.addEventListener('submit', async (e) => {
                e.preventDefault();
                if (!this.currentUser) { this.showToast('Devi essere loggato.', { type: 'error' }); return; }
        ...
        if (editScoutForm && !editScoutForm._bound) {
            ...
            editScoutForm.addEventListener('submit', async (e) => {
                ...
                if (!validation.valid) { this.showToast(Object.values(validation.errors)[0], { type: 'error' }); return; }
                ...
                if (this.checkDuplicateScout(nome, cognome, id)) {
                    this.showToast('Esploratore già esistente.', { type: 'error' }); return;
                }
                ...
                try {
                    ...
                    console.error(error); this.showToast('Errore', { type: 'error' });
                } finally {
                    this.setButtonLoading(submitBtn, false, originalText);
                }
            });
        ...
        if (confirmDeleteScoutButton && !confirmDeleteScoutButton._bound) {
            ...
            confirmDeleteScoutButton.addEventListener('click', async () => {
                ...
                } catch (e) { console.error(e); this.showToast('Errore', { type: 'error' }); }
                finally { this.setButtonLoading(confirmDeleteScoutButton, false, originalText); }
            });
        }
        ...
        if (editActivityForm && !editActivityForm._bound) {
            ...
            editActivityForm.addEventListener('submit', async (e) => {
                ...
                finally { this.setButtonLoading(submitBtn, false, originalText); }
            });
        }
    ...
    loadUserPreferences() {
        try { const local = localStorage.getItem('app-preferences'); if (local) return JSON.parse(local); } catch (e) { }
    ...
    async syncUserPreferences() {
        ...
        try {
            ...
            if (snap.exists()) {
                ...
                if (merged.theme) this.applyTheme(merged.theme);
    ...
    renderCommentsList(comments, container) {
        if (!container) return;
        if (!comments.length) { container.innerHTML = '<p class="text-sm text-gray-500">Nessun commento.</p>'; return; }
    ...
    async saveFCMToken(token) {
        if (this.currentUser?.uid) await setDoc(doc(DATA.adapter.db, 'fcm-tokens', this.currentUser.uid), { token, userId: this.currentUser.uid, updatedAt: new Date() }, { merge: true });
    },
    ...
    async markAllNotificationsAsRead() {
        ...
        for (const n of unread) await updateDoc(doc(DATA.adapter.db, 'in-app-notifications', n.id), { read: true });
        this.updateNotificationsBadge();
