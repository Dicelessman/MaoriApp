 ...
 */
export function checkDataIntegrity(state: any): any {
    if (!state) {
        return {
            isValid: true,
            summary: {
    ...
    const scoutIdSet = new Set(scouts.map((s: any) => String(s.id)));
    const activityIdSet = new Set(activities.map((a: any) => String(a.id)));

    // 1. Orphan Presences
    const orphanPresencesNoScout: any[] = [];
    const orphanPresencesNoActivity: any[] = [];
    ...
    for (const [key, group] of scoutMap.entries()) {
        if (group.length > 1) {
            duplicateScouts.push({
                key,
                count: group.length,
                scouts: group.map((s: any) => ({ id: s.id, nome: s.nome, cognome: s.cognome, pv_pattuglia: s.pv_pattuglia }))
            });
        }
    }

    // 3. Duplicate Staff Emails
    const duplicateStaffEmails: any[] = [];
    ...
    for (const [email, group] of staffMap.entries()) {
        if (group.length > 1) {
            duplicateStaffEmails.push({
                email,
                count: group.length,
                staff: group.map((m: any) => ({ id: m.id, nome: m.nome, cognome: m.cognome, email: m.email }))
            });
        }
    }

    // 4. Invalid Date Activities
    const invalidDateActivities: any[] = [];
    ...
    const uniqueOrphanPresenceIds = new Set([
        ...orphanPresencesNoScout.map((p: any) => p.id || `${p.esploratoreId}_${p.attivitaId}`),
        ...orphanPresencesNoActivity.map((p: any) => p.id || `${p.esploratoreId}_${p.attivitaId}`)
    ]);
